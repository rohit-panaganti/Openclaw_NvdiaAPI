# Openclaw + NVIDIA NIM API — Telegram Bot
...
```
Telegram user → @YourBot → OpenClaw Gateway (local) → NVIDIA NIM API → reply
```
...
# 4. Edit the config — fill in your API keys and IDs
...
# 3. Edit the config — fill in your API keys
...
| `YOUR_GATEWAY_AUTH_TOKEN` | Any random string — run `openssl rand -hex 24` |
...
### Slow response times (15–100 seconds)

This is normal. Three sources of latency:
1. **Telegram polling** — openclaw polls every ~50 seconds; your message may wait up to 50s to be seen
2. **NVIDIA cold start** — first request after idle takes 10–30s to warm up the model
3. **Model inference** — Minimax M2.7 takes 5–20s to generate a response
...
The gateway registers itself as a login item automatically when you run `gateway start`. It will restart every time you log in — no manual start needed.
